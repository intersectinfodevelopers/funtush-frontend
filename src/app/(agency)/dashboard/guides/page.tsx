...
export default function GuidesPage() {
  ...
  const nextExpiry = guidesData
    .flatMap((guide) =>
      guide.certifications.map((cert) => ({ guide: guide.name, ...cert })),
    )
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-800">
                {nextExpiry
                  ? `${nextExpiry.guide}'s certification expires on ${new Date(nextExpiry.expiry).toLocaleDateString()}.`
          ...
          <tbody>
            {paginatedGuides.map((guide, index) => {
              ...
              return (
                <tr
                  key={guide.id}
                  className="border-b border-neutral-200 transition hover:bg-slate-50"
                >
                  ...
                  <td className="px-4 py-4 text-xs leading-5 text-neutral-600">
                    {guide.certifications.length > 0 ? (
                      guide.certifications.map((cert) => (
                        <div
                          key={cert.number}
                          className="mb-2 rounded-2xl bg-slate-50 px-3 py-2"
                        >
                          <div className="font-semibold text-neutral-900">
                            {cert.name}
                          </div>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        title="Edit guide"
                        onClick={() => setDialog({ type: "edit", guide })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-neutral-900">
              {dialog.type === "delete" ? "Delete guide?" : "Edit guide?"}
            </h2>
            <p className="mt-2 text-sm leading-6 text-neutral-600">
              {dialog.type === "delete"
                ? `Remove ${dialog.guide.name} from the guide list?`
                : `Open ${dialog.guide.name}'s profile to edit?`}
            </p>
