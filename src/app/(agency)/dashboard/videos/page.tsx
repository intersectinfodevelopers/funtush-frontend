...
export default function VideosPage() {
  ...
  const rows = useMemo<Video[]>(() => videosData.map((v) => ({ ...v })), []);
    ...
    <div className="space-y-4 w-full min-h-screen">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/dashboard/videos/new" className="inline-flex items-center gap-2 rounded-xl bg-primary-600 px-5 py-3 text-sm font-semibold text-white hover:bg-primary-700">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <AnalyticsSummaryCard label="Total Videos" value={stats.total} tone="primary" icon={() => null} />
        <AnalyticsSummaryCard label="Active" value={stats.active} tone="success" icon={() => null} />
        <AnalyticsSummaryCard label="Inactive" value={stats.inactive} tone="danger" icon={() => null} />
      </div>
      ...
      <section className="overflow-x-auto border-t border-neutral-200 bg-white">
        <table className="min-w-full border-collapse text-left text-sm text-neutral-700">
          ...
          <tbody>
            {paginated.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-sm text-neutral-500">No videos found.</td>
              </tr>
            ) : (
              paginated.map((video, idx) => (
                <tr key={video.id} className="border-b border-neutral-200 hover:bg-neutral-50">
                  <td className="px-4 py-3">{(safePage - 1) * perPage + idx + 1}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="h-12 w-20 rounded bg-neutral-100 flex-shrink-0 bg-cover" style={{ backgroundImage: `url(${video.thumbnail})` }} />
                      <div>
                        <div className="font-semibold">{video.title}</div>
                        <div className="text-xs text-neutral-500">{video.youtubeUrl}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-neutral-700">{video.description}</td>
                  <td className="px-4 py-3">{video.order}</td>
                  <td className="px-4 py-3 text-neutral-700">{video.status}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button aria-label={`Preview ${video.title}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-primary-50 text-primary-700">
                        <Eye className="h-4 w-4" />
                      </button>
                      <Link href={`/dashboard/videos/${video.id}/edit`} className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-warning-50 text-warning-700">
                        <Edit3 className="h-4 w-4" />
                      </Link>
                      <button aria-label={`Delete ${video.title}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-danger-50 text-danger-700">
