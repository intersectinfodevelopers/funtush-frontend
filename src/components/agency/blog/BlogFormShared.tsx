  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid w-full grid-cols-1 items-start gap-4 lg:grid-cols-2">
        {/* Editor Section (includes publish settings) */}
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className={`w-full rounded-xl border px-3.5 py-3 text-xs shadow-sm focus:outline-none focus:ring-2 transition-colors ${errors.title ? errorInputClass : inputClass}`}
                />
              ...
              <div className="mt-4 space-y-1.5">
                ...
                <input
                  ...
                  className={`w-full rounded-xl border px-3.5 py-3 text-xs shadow-sm focus:outline-none focus:ring-2 ${inputClass}`}
                />
              </div>
              ...
              <div className="mt-4 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    className={`flex items-center gap-1.5 text-[11px] font-medium transition-colors ${isDark ? "text-primary-400 hover:text-primary-300" : "text-primary-500 hover:text-primary-600"}`}
                  >
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className={`w-full rounded-xl border px-3.5 py-3 text-xs shadow-sm focus:outline-none focus:ring-2 ${errors.category ? "border-danger-500 focus:ring-danger-500" : `${selectClass} focus:border-primary-500 focus:ring-primary-500`}`}
                  >
                  ...
                  <div className="space-y-1.5 animate-fadeIn">
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        className={`w-full rounded-xl border px-3.5 py-3 pr-10 text-xs shadow-sm focus:outline-none focus:ring-2 ${errors.publishDate ? "border-danger-500 focus:ring-danger-500" : `${inputClass} focus:border-primary-500 focus:ring-primary-500`}`}
                      />
                ...
                <div className="space-y-1.5">
                  ...
                  <div className={`w-full rounded-xl border px-3 py-2 text-xs shadow-sm ${inputClass}`}>
                    <div className="flex flex-wrap gap-1.5 mb-1.5">
                      {tagsList.map((t, index) => (
                        <span key={index} className="inline-flex items-center gap-1 bg-primary-500 text-white text-[11px] px-2 py-0.5 rounded-md">
                          {t}
                          <button type="button" onClick={() => handleRemoveTag(index)} className="hover:text-danger-200 font-bold ml-0.5 transition-colors">&times;</button>
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <button type="button" onClick={() => handleAddTag(tagInput)} className="shrink-0 rounded-lg bg-primary-900 px-3 py-1 text-xs font-medium text-white transition-colors hover:bg-primary-800">Add</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  <label className="block font-bold text-xs">Photos <span className="text-danger-500">*</span></label>
                  ...
                  <div className="flex flex-col gap-2 text-xs sm:flex-row sm:items-center sm:gap-5">
                    <label className={`flex cursor-pointer items-center gap-1.5 ${isDark ? "text-neutral-200" : "text-neutral-700"}`}>
                      <input type="radio" name="photoSource" checked={photoOption === "local"} onChange={() => handlePhotoOptionChange("local")} className="accent-primary-500" />
                      <span>Upload from Local Drive</span>
                    </label>

                    <label className={`flex cursor-pointer items-center gap-1.5 ${isDark ? "text-neutral-200" : "text-neutral-700"}`}>
                      <input type="radio" name="photoSource" checked={photoOption === "gallery"} onChange={() => handlePhotoOptionChange("gallery")} className="accent-primary-500" />
                  ...
                  {photoOption === "local" && (
                    <>
                      {!selectedFile && (
                        <div className={`border border-dashed rounded-2xl p-6 text-center flex flex-col items-center justify-center gap-3 ${errors.photo ? "border-danger-500" : isDark ? "border-neutral-700 bg-neutral-800/50" : "border-neutral-300 bg-neutral-50"}`}>
                          ...
                          <label className="cursor-pointer rounded-xl bg-primary-900 px-4 py-2 text-xs font-medium text-white shadow-md shadow-primary-900/20 transition-colors hover:bg-primary-800">
                            Upload Image
                            <input type="file" accept="image/jpeg,image/png,image/webp,image/jpg" onChange={handleFileChange} className="hidden" />
                          </label>
                        </div>
                      )}
                      ...
                      {selectedFile && (
                        <div className="relative rounded-xl overflow-hidden border border-primary-500/30 w-full">
                          <Image src={URL.createObjectURL(selectedFile)} alt={selectedFile.name} width={800} height={450} className="w-full h-32 object-cover" />
                          <div className={`p-2 text-xs flex justify-between items-center ${isDark ? "bg-neutral-900 text-neutral-200" : "bg-white text-neutral-800"}`}>
                            ...
                            <button type="button" onClick={() => setSelectedFile(null)} className="text-danger-500 hover:underline font-medium text-[11px]">Remove</button>
                          </div>
                        </div>
                      )}

                      {/* show photo error in this section as requested */}
                  ...
                  {photoOption === "gallery" && (
                    <div className={`rounded-2xl border p-4 ${isDark ? "border-neutral-700 bg-neutral-800/50" : "border-neutral-200 bg-neutral-50"}`}>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button type="button" onClick={() => setShowGalleryModal(true)} className="rounded-lg bg-primary-900 px-3 py-1 text-xs font-medium text-white transition-colors hover:bg-primary-800">Open Gallery</button>
                      </div>

                      {selectedGalleryImage ? (
                        <div className="relative rounded-xl overflow-hidden border border-primary-500/30">
                          <Image src={selectedGalleryImage.url} alt={selectedGalleryImage.title || "Selected image"} width={800} height={450} className="w-full h-32 object-cover" />
                          <div className={`p-2 text-xs flex justify-between items-center ${isDark ? "bg-neutral-900 text-neutral-200" : "bg-white text-neutral-800"}`}>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-6 md:grid-cols-3 gap-3 min-h-60">
